...
class Listpeserta extends MY_Controller{
    public function index(){  
        ...
        if(!$this->ion_auth->is_admin()){
            ...
            //var_dump($user);
            $defaultFilter = (object)array('column' => 'AdminAssignment', 'value' => $user->id);
        }
        ...
        if($filterColumn && $pageSelected && $pageSizeSelected){
            $data['dataInfo'] = $this->pesertamodel->get_data_byfilterpage($pageSelected, $pageSizeSelected, $filterColumn, $filterValue, $filterOperator, $sortBy, $sortOrder,$defaultFilter);
        }else{
            $data['dataInfo'] = $this->pesertamodel->get_data_byfilterpage(1,10,'','','','', '',$defaultFilter);
        }
